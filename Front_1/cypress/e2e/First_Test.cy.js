describe('Smoke Test: Home Page Elements', () => {
  it('Loads the Home Page and verifies key elements', () => {
    // Intercept the initial load to ensure the page renders even if backend is down
...
// --- FLOW 4: Happy Path (Comment on Event) ---
  ...
  // --- FLOW 7: Happy Path (Host Flow) ---
  // Mapped to Story S-4 (Event Fields) and S-1 (View Event)
  it('Flow 7 (Happy): Event gets edited by its Host', () => {
    ...
    cy.intercept('GET', '**/search*', {
      ...
      body: {
        ...
        data: {
          ...
          events: [
            {
              id: 101, // Fake ID for the new event
    ...
    // Submit Form (Matches S-4 "Press Done")
    cy.contains('button', 'Done').click(); 
    ...
    // 2. Click result (Target heading to avoid clicking input)
    cy.wait('@searchNewEvent'); // Ensure search results are loaded
    cy.contains('h4', 'Winter Gala Extravaganza').should('be.visible').click();

    // 3. Verify Event Details page loaded
    cy.wait('@getNewEventDetails'); // Ensure details are loaded
